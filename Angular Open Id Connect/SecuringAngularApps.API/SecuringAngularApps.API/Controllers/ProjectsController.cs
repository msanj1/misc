...
namespace SecuringAngularApps.API.Controllers
{
    ...
    public class ProjectsController : Controller
    {
        ...
        public IEnumerable<Project> GetProjects()
        {
            ...
            else
            {
                ...
                List<int> userProjectIds = _context.UserPermissions.Where(up =>
                        up.ProjectId.HasValue && up.UserProfileId == userId)
        ...
        {
            var userId = this.User.FindFirstValue(JwtClaimTypes.Subject);
            var userAccess = await _context.UserPermissions.FirstOrDefaultAsync(up => up.ProjectId == projectId
        ...
        public async Task<bool> MilestoneAccessCheck(Milestone milestone)
        {
            var userId = this.User.FindFirstValue(JwtClaimTypes.Subject);
            var perm = await _context.UserPermissions.FirstOrDefaultAsync(up => up.ProjectId == milestone.ProjectId
                                                                                && up.UserProfileId == userId);
